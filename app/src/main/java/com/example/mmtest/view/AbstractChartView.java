...
import com.example.mmtest.ViewportChangeListener;
import com.example.mmtest.animation.ChartAnimationListener;
import com.example.mmtest.animation.ChartDataAnimator;
import com.example.mmtest.animation.ChartDataAnimatorV14;
import com.example.mmtest.animation.ChartDataAnimatorV8;
import com.example.mmtest.animation.ChartViewportAnimator;
import com.example.mmtest.animation.ChartViewportAnimatorV14;
import com.example.mmtest.animation.ChartViewportAnimatorV8;
import com.example.mmtest.gesture.ChartTouchHandler;
...
import com.example.mmtest.renderer.AxesRenderer;
...
public abstract class AbstractChartView extends View implements Chart {
	...
	protected void onSizeChanged(int width, int height, int oldWidth,
		...
		chartComputator.setContentArea(getWidth(), getHeight(),
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (isInteractive) {
			...
			Log.v("AbstractChartView: onTouchEvent", " isInteractive: "
					+ isInteractive + " isContainerScrollEnabled:"
					+ isContainerScrollEnabled + "  needInvalidate:"
					+ needInvalidate);
			if (needInvalidate) {
	...
	public void setZoomLevel(float x, float y, float zoomLevel,
		...
		if (zoomViewport.contains(x, y)) {
			...
			zoomViewport.set(x - halfWidth, y + halfHeight, x + halfWidth, y
					- halfHeight);
